<template>
  <div class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
    <div class="bg-white p-6 rounded-lg shadow-lg  relative">
      <!-- Close Button -->
      <button @click="closeForm" class="absolute top-2 right-2 text-gray-500 text-xl">
        <ion-icon name="close-outline"></ion-icon>
      </button>

      <!-- Address Display -->
      <div class="text-center mb-6">
        <p class="font-bold text-lg">Địa chỉ đã chọn</p>
      ...
      <div class="flex">
        ...
        <div class="space-y-2 mr-4">
          <label for="province" class="block text-sm font-medium">Tỉnh thành</label>
          <div class="relative">
            <select 
              v-model="selectedProvince" 
              @change="fetchDistricts" 
              class="w-full px-3 py-2 border border-gray-300 rounded-md w-auto focus:outline-none"
              >
            ...
            <div v-if="loadingProvince" class="absolute top-2 right-2">
        ...
        <div class="space-y-2 mr-4">
          ...
          <div class="relative">
            <select v-model="selectedDistrict" @change="fetchCommunes" class="w-full px-3 py-2 border border-gray-300 rounded-md w-auto focus:outline-none">
              <option value="">--Chọn quận huyện--</option>
              <option v-for="district in districts" :key="district.idDistrict" :value="district.idDistrict">
                {{ district.name }}
              </option>
            </select>
            <div v-if="loadingDistrict" class="absolute top-2 right-2">
              <img src="https://icons8.com/preloaders/preloaders/1488/Iphone-spinner-2.gif" alt="Loading" />
            </div>
          </div>
        </div>
        ...
        <!-- Commune Selector -->
        <div class="space-y-2">
          <label for="commune" class="block text-sm font-medium">Phường xã</label>
          <div class="relative">
            <select v-model="selectedCommune" class="w-full px-3 py-2 border border-gray-300 rounded-md w-auto focus:outline-none">
              <option value="">--Chọn phường xã--</option>
              <option v-for="commune in communes" :key="commune.idCommune" :value="commune.idCommune">
                {{ commune.name }}
              </option>
            </select>
            <div v-if="loadingCommune" class="absolute top-2 right-2">
              <img src="https://icons8.com/preloaders/preloaders/1488/Iphone-spinner-2.gif" alt="Loading" />
            </div>
          </div>
        </div>
      ...
      <!-- Complete Button -->
      <button @click="completeSelection" class="flex items-center justify-center w-full mt-6 py-3 bg-green-500 text-white rounded-md hover:bg-green-600">
        <ion-icon class="mr-4 text-2xl" name="checkmark-outline"></ion-icon>
        Hoàn thành
      </button>
    </div>
  </div>
