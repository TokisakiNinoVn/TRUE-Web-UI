<template>
  <header
    class="bg-gray-800 text-white p-4 flex items-center justify-between sticky top-0 left-0 w-full z-50"
  >
    <!-- App Name -->
    <div class="text-2xl font-bold">
      <router-link to="/" class="text-white hover:text-gray-300"
        >TroThaiNguyen</router-link
      >
    </div>
    <nav class="flex items-center space-x-6">
      ...
      <div class="relative group">
        <ion-icon
          ...
          class="text-2xl cursor-pointer hover:text-gray-300"
        ></ion-icon>
        <!-- Notification Dropdown -->
        <ul
          class="absolute right-0 hidden group-hover:block bg-gray-700 rounded-lg shadow-lg mt-2 w-64"
        >
      ...
      <ul class="flex space-x-6">
        ...
        <li class="flex items-center">
          <router-link to="/admin-post" class="hover:text-gray-300"
            >Admin Post</router-link
          >
        </li>
        <li v-if="!isLoggedIn">
        ...
        <li v-if="!isLoggedIn">
          <router-link to="/signup" class="hover:text-gray-300"
            >Đăng ký</router-link
          >
        </li>
        <li v-if="isLoggedIn" class="relative group">
          ...
          >
            ...
            <img
              :src="baseURL + userInfor.avatar.imageUrl"
              alt="avatar"
              class="w-10 h-10 rounded-full object-cover"
            />
          </router-link>
          <ul
            class="absolute left-0 hidden group-hover:block bg-gray-700 rounded-lg shadow-lg mt-2"
          >
            <li>
              <router-link
                :to="`/user/${userInfor.username}`"
                class="block px-4 py-2 hover:bg-gray-600 flex items-center"
            ...
            <li>
              <a
                ...
                class="block px-4 py-2 hover:bg-gray-600 flex items-center"
                ><ion-icon name="log-out-outline" class="mr-2"></ion-icon> Đăng
                xuất</a
              >
            </li>
          </ul>
    ...
    >
      <div class="bg-white p-6 rounded-lg shadow-lg max-w-sm w-full">
        ...
        <div class="flex justify-end space-x-4">
          ...
          <button
            @click="cancelLogout"
            class="bg-gray-300 text-gray-800 px-4 py-2 rounded-lg hover:bg-gray-400 transition duration-150 ease-in-out"
          >
            Không
          </button>
        </div>
...
onMounted(() => {
  ...
  if (loggedInStatus && accountInfo) {
    ...
    userInfor.value = {
      ...
      avatar: parsedAccount.userInfor.avatar || {
