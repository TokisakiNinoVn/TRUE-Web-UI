<template>
  <div class="p-4">
    <h2 class="text-2xl font-bold mb-4">
      <ion-icon name="pricetag-outline"></ion-icon>Giá rẻ
    </h2>
    ...
    <div class="flex flex-wrap gap-4" v-if="posts.length">
      <div
        v-for="post in posts"
        :key="post._id"
        class="one-item bg-white border border-gray-200 rounded-lg shadow-md overflow-hidden max-w-[300px] transition-transform transform hover:scale-105 hover:shadow-lg"
      >
        <div class="relative w-full pb-[125%]">
          <img
            :src="getImageUrl(post.file[0]?.fileUrl)"
            alt="Post Image"
...
const loadLowestPricedPosts = async () => {
  try {
    await postStore.getLowestPricedPostsAction();
    posts.value = postStore.lowestPricedPosts;
  } catch (err) {
    error.value =
      err.response?.data?.message || "Failed to fetch lowest priced posts";
  } finally {
    loading.value = false;
  }
};
