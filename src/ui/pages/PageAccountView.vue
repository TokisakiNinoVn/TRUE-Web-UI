<template>
  ...
  >
    <div class="card bg-white w-full max-w-4xl p-6 rounded-lg shadow-md">
      ...
      <form
        class="main-form flex flex-col space-y-4"
        ...
        >
          <div class="form-left w-full md:w-3/4">
            <table class="w-full">
              <tbody>
                <tr>
                  <td class="text-right pr-4 font-medium text-gray-700">
          ...
          <div
            class="form-right w-1/3 flex flex-col justify-center space-y-4 md:space-y-0 md:items-center"
          >
            <img
...
import CNavAccount from "@/ui/components/CNavAccount.vue";
...
onMounted(() => {
  ...
  if (accountInfo) {
    ...
    if (parsedAccount.username === username) {
      ...
      formattedBirthday.value = moment(userInfo.value.birthday).format(
        "DD/MM/YYYY"
      );
    }
  }
